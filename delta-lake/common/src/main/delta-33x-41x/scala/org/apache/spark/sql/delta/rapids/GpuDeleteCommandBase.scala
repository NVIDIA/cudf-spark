...
import org.apache.spark.sql.delta.commands.{DeleteCommandMetrics, DeleteMetric,
  DeletionVectorUtils, DeltaCommand}
...
/**
 * Base class for GPU version of Delta Lake DeleteCommand.
...
abstract class GpuDeleteCommandBase(
          ...
          if (shouldWriteDVs) {
            ...
            if (filesToRewrite.isEmpty) {
              ...
              val targetDF = RowTracking.preserveRowTrackingColumns(
                dfWithoutRowTrackingColumns = createDataFrame(sparkSession, newTarget),
  ...
  def shouldWritePersistentDeletionVectors(
      spark: OperationSparkSession, txn: OptimisticTransaction): Boolean = {
