...
abstract class GpuCreateDeltaTableCommandBase(
  ...
  private def handleCommit(
    ...
    val txn = startTxnForTableCreation(sparkSession, gpuDeltaLog, tableWithLocation)
    ...
    OptimisticTransaction.withActive(txn) {
      val result = query match {
        ...
        case Some(cmd: CloneTableCommand) =>
          ...
          cmd.handleClone(
            sparkSession,
            txn,
            targetDeltaLog = gpuDeltaLog.deltaLog,
      ...
      runPostCommitUpdates(sparkSession, txn, gpuDeltaLog, tableWithLocation)
  ...
  private def runPostCommitUpdates(
    ...
    logInfo(
      log"Table is path-based table: " +
        log"${DeltaMdcShims.mdc(DeltaLogKeys.IS_PATH_TABLE, tableByPath)}. " +
        log"Update catalog with mode: " +
        log"${DeltaMdcShims.mdc(DeltaLogKeys.OPERATION, operation)}")
    val opStartTs = TimeUnit.NANOSECONDS.toMillis(txnUsedForCommit.txnStartTimeNs)
    val postCommitSnapshot = gpuDeltaLog.deltaLog.update(
    ...
    if (UniversalFormat.icebergEnabled(postCommitSnapshot.metadata) &&
      ...
      gpuDeltaLog.deltaLog.icebergConverter.convertSnapshot(postCommitSnapshot, tableWithLocation)
    }

    if (UniversalFormat.hudiEnabled(postCommitSnapshot.metadata) &&
      !txnUsedForCommit.containsPostCommitHook(HudiConverterHook)) {
      gpuDeltaLog.deltaLog.hudiConverter.convertSnapshot(postCommitSnapshot, tableWithLocation)
    }
  }
  ...
  private def handleCreateTableAsSelect(
    ...
    val updatedConfiguration = enforceDependenciesInConfiguration(
      sparkSession, tableWithLocation, deltaWriter.configuration, txn.snapshot)
  ...
  private def handleCreateTable(
    ...
    def createActionsForNewTableOrVerify(): Seq[Action] = {
      ...
      if (noExistingMetadata && !allowRestoringExistingEmptySchemaTable) {
        ...
        var newMetadata = getProvidedMetadata(tableWithLocation, table.schema.json)
        val updatedConfig = enforceDependenciesInConfiguration(
          sparkSession,
          tableWithLocation,
  ...
  private def cleanupTableDefinition(spark: SparkSession,
     table: CatalogTable,
     snapshot: Snapshot): CatalogTable = {
   ...
   * @param gpuDeltaLog Delta log used for logging purposes.
   */
