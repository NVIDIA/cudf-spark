...
class ProjectSplitRetrySuite extends RmmSparkRetrySuiteBase {
  import ProjectSplitRetrySuite.TestColumnarLeafExec
  ...
  private def runProjectSplitRetry(
      buildProject: TestColumnarLeafExec => GpuExec): Int = {
    ...
    try {
      ...
      closeOnExcept(input) { _ =>
        ...
        val project = buildProject(TestColumnarLeafExec(batchAttrs, inputRdd))
  ...
  test("GpuProjectAstExec streams split-retry pieces and counts output batches") {
    val numBatches = runProjectSplitRetry { child =>
      GpuProjectAstExec(
        addOneExprs().map(_.asInstanceOf[NamedExpression]).toList,
        child)()
    }
    assert(numBatches >= 2,
      s"expected >= 2 pieces from GpuProjectAstExec, got $numBatches")
  }

  test("literal-only GpuProjectAstExec streams split-retry pieces") {
    val numBatches = runProjectSplitRetry { child =>
      GpuProjectAstExec(
        List(GpuAlias(GpuLiteral(7, IntegerType), "literal")()),
        child)()
    }
    assert(numBatches >= 2,
      s"expected >= 2 pieces from literal-only GpuProjectAstExec, got $numBatches")
  }
