...
abstract class BaseExprMeta[INPUT <: Expression](
  ...
  final def canBePrecomputedForJoin: Boolean =
    canExprTreeBeReplaced && !willUseGpuCpuBridge &&
      convertedGpuExpression.forall(!_.hasSideEffects)
  ...
  // Keep this dynamic for the same bridge-state reason as canSelfBeAst.
  final def canSelfBeLegacyAst: Boolean = {
    tagForAst()
    canThisBeReplaced && !willUseGpuCpuBridge &&
